...
### 2.3.1 Primer K-Means
#### 2.3.1.1 Análisis previo del Kmeans
```{r}
...
#### 2.3.1.2 Mapa de K-Means 1 y nuevo grupo
```{r}
...
### 2.3.2 Segundo K-Means
Ahora realizamos un kmeans dentro del grupo mejor evaluado para seguir reduciendo hasta llegar a grupos de 5 paises
```{r echo=FALSE}
...
#### 2.3.2.1 Análisis previo del Kmeans
```{r}
...
#### 2.3.2.2 Mapa de K-Means 2 y nuevo grupo
```{r}
...
## 3.1 Primer Grupo
ESTOS AHORITA NO, LUEGO CONSEGUIR BIEN LOS DATOS EN BLOOMBERG
POR EL MOMENTO USAR 
...
Equity1_Portfolio <- merge.xts(get(Equity1_ad[1]),
                              get(Equity1_ad[2]),
                              join = "inner")

for(i in 3:length(Equity1_ad)){
  Equity1_Portfolio <- merge.xts(Equity1_Portfolio,
                                get(Equity1_ad[i]),
                                join = "inner")
}
Equity1_Returns <- Return.calculate(Equity1_Portfolio)[-1,]
View(Equity1_Returns)
#----
Equity2 <- c("^OMXCXC20PI", "CES300.HK", "^KS11", "^NZ50", "OSEBX.OL", "WISGP.SI",
             "^OMX", "^TSE50", "^FTSE", "^GSPC")
getSymbols(Equity1, from=as.Date("2018-01-01"), to=Sys.Date())
...
Equity2_Returns <- Return.calculate(Equity2_Portfolio)[-1,]
View(Equity2_Returns)
```
...
Estg1
```
### 3.1.1 Estadisticas Descrriptivas de los Retornos
### 3.1.2 Pruebas de Normalidad Univariadas
#### 3.1.2.1 Prueba de Augmented Dickey-Fuller
```{r}
...
```
### 3.1.3 Pruebas de Normalidad Multivariadas
#### 3.1.3.1 Prueba de Kormogorov-Smirnov Multivariada
```{r}
...
### 3.2.1 Estadisticas Descrriptivas de los Retornos
### 3.2.2 Pruebas de Normalidad Univariadas
#### 3.2.2.1 Prueba de Augmented Dickey-Fuller
#### 3.2.2.2 Pruba de Kormogorov-Smirnov
### 3.2.3 Pruebas de Normalidad Multivariadas
### 3.2.4 Portafolio Optimizado con Distribución Normal
### 3.2.5 Portafolio Optimizado con Distribución NIG
...
# PARTE . ¿SON LOS PORTAFOLIOS UN GRUPO DE INVERSIÓN?
A travez de formación de portafolios probar si un grupo de activos son un grupo de inversión o no.
# PARTE . MODELOS DE REGRESIÓN PARA EXPLICAR LAS RAZONES DE SHARP DE LOS GRUPOS
